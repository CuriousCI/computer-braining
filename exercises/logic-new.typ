...
#let logic(body) = {
    ...
    show regex("\b[a-z_]{2,}"): it => emph(it)
...
#let operation(
    ...
    highlight[
        #figure(kind: "operation", supplement: to-string(name), logic[#name\(#args)#if type != none [: #type]])
        #label(to-string(name))
        #label("operation")
    ]
...
#let extension(body: none, new-objects: none, old-objects: none, tuple-variations: none) = {
    sub-par([modifiche al livello estensionale:], {
        if body != none {
            v(2pt)
            body
        }
